...
public class PackageResourceLoader  {
	...
	public Resource[] getResources(String basePackage) throws IOException {
		...
		File rootDir = new File(url.getFile());//获取根目录
	...
	protected Set<File> retrieveMatchingFiles(File rootDir) throws IOException {
		...
		*/
		Set<File> result = new LinkedHashSet<File>(8);//为什么这个长度为8
	...
	protected void doRetrieveMatchingFiles( File dir, Set<File> result) throws IOException {
		...
		File[] dirContents = dir.listFiles();//目录下编译后的class文件  E:\GitHub\litespring\target\test-classes\org\litespring\dao\v4\AccountDao.class, E:\GitHub\litespring\target\test-classes\org\litespring\dao\v4\ItemDao.class
		if (dirContents == null) {
			if (logger.isWarnEnabled()) {
